        ...
        <Container size="4xl" className="relative z-10 text-center">
          ...
          >
             ...
             <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex items-center overflow-hidden gap-4">
                <Search className="w-6 h-6 text-slate-400 dark:text-slate-300 ml-5 flex-shrink-0 gap-4" />
                <input 
                  type="text" 
                ...
                />
                <div className="hidden sm:flex items-center gap-1 pr-5 text-xs font-medium text-slate-400 dark:text-slate-300 bg-transparent">
          ...
          <motion.div 
            ...
            className="mt-8 flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-slate-400 dark:text-slate-300"
          >
             <span className="text-slate-500 dark:text-slate-400 dark:text-slate-300">Popular topics:</span>
      ...
      <Container size="7xl" className="-mt-16 relative z-20 pb-24">
                 ...
                 {docsData.categories.map((cat, idx) => (
                    ...
                    >
                      <Card className="h-full hover:border-primary-500/50 dark:hover:border-primary-500/50 group bg-white dark:bg-slate-900/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-shadow" hoverEffect>
                         <div className="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-600 dark:text-slate-400 dark:text-slate-300 mb-6 group-hover:bg-primary-500 group-hover:text-white transition-all duration-300 shadow-sm group-hover:shadow-primary-500/30 gap-4">
           ...
           <Grid cols={5} gap={16} className="relative z-10">
              <div className="lg:col-span-2">
                 ...
                 <div className="flex items-center gap-4 mb-8">
                    <div className="relative">
                       ...
                       <ChevronRight className="w-4 h-4 text-slate-400 dark:text-slate-300 absolute right-3 top-2.5 rotate-90 pointer-events-none" />
                    </div>
                    <span className="inline-block w-2 h-2 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]"></span>
                    <Typography variant="caption" className="text-slate-400 dark:text-slate-300">All systems operational</Typography>
                 </div>

                 <Typography variant="body-lg" className="text-slate-400 dark:text-slate-300 leading-relaxed">Integrasikan BizOps dengan aplikasi pihak ketiga Anda. Dokumentasi lengkap untuk REST API, Webhooks, dan SDK tersedia.</Typography>
                 ...
                 <Stack direction="vertical" gap={3}>
                    ...
                    <Link to="/docs/changelog" className="text-slate-400 dark:text-slate-300 hover:text-white text-sm font-medium flex items-center gap-2 mt-2">
              ...
              <div className="lg:col-span-3">
                 <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-2xl overflow-hidden">
                    <div className="flex justify-between items-center px-4 py-3 bg-slate-900 border-b border-slate-700 gap-4">
                       ...
                       <div className="flex items-center gap-4">
                          <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-300">bash</span>
                          <button onClick={handleCopy} className="text-xs flex items-center gap-1.5 text-slate-400 dark:text-slate-300 hover:text-white transition-colors group">
                             {copied ? <Check className="w-3 h-3 text-green-400 dark:text-green-300" /> : <Copy className="w-3 h-3 group-hover:text-blue-400 dark:text-blue-300" />}
                             {copied ? <span className="text-green-400 dark:text-green-300">Copied!</span> : "Copy"}
                          </button>
                       </div>
                    </div>
                    <div className="p-6 text-sm leading-relaxed overflow-x-auto">
                       <div className="text-slate-300">
                          <span className="text-purple-400 dark:text-purple-300">curl</span> --request GET \<br/>
                          &nbsp;&nbsp;--url <span className="text-green-400 dark:text-green-300">'https://api.bizops.id/v2/resource/Employee'</span> \<br/>
                          &nbsp;&nbsp;--header <span className="text-green-400 dark:text-green-300">'Authorization: Token xyz:123'</span> \<br/>
                          &nbsp;&nbsp;--header <span className="text-green-400 dark:text-green-300">'Content-Type: application/json'</span>
                       </div>
                       
                       <div className="mt-6 pt-6 border-t border-slate-700/50">
                          <div className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-300 mb-2 uppercase tracking-wider">JSON Response</div>
                          <div className="text-blue-300">
                             ...
                             &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-slate-500 dark:text-slate-400 dark:text-slate-300">...</span><br/>
                             &nbsp;&nbsp;]<br/>
                             {`}`}
                          </div>
                       </div>
                 ...
                 <Grid cols={4} gap={4} className="mt-6">
                    {['Authentication', 'Webhooks', 'Rate Limits', 'Errors'].map(item => (
                       <div key={item} className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs font-medium text-slate-400 dark:text-slate-300 text-center cursor-pointer transition-colors">
                          {item}
                       </div>
                    ))}
                 </Grid>
        ...
        <div>
           <div className="text-center mb-12">
             ...
             <Typography variant="body" className="text-slate-600 dark:text-slate-400 dark:text-slate-300">Our support team and community are here for you.</Typography>
           </div>
           ...
           <div className="md:hidden">
              <CardSlider mobileItemWidth="w-[85vw] sm:w-[350px]">
                 <a href="https://discord.gg/bizops" target="_blank" rel="noopener noreferrer" className="block h-full">
                    <Card className="h-full flex flex-col items-center text-center hover:border-indigo-400 dark:hover:border-indigo-500 transition-all duration-300 shadow-lg hover:shadow-xl transition-shadow gap-4" hoverEffect>
                       <div className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 dark:text-indigo-300 rounded-2xl flex items-center justify-center mb-6 shadow-sm gap-4">
                          <MessageSquare className="w-8 h-8" />
                       </div>
                       <Typography variant="h3" as="h3">Developer Community</Typography>
                       <Typography variant="caption" className="text-slate-500 dark:text-slate-400 dark:text-slate-300">Join 2,000+ developers. Discuss API integration, share custom scripts, and get help.</Typography>
                       <span className="text-indigo-600 dark:text-indigo-400 dark:text-indigo-300 font-bold text-sm">Join Discord &rarr;</span>
                    </Card>
                 </a>
                 
                 <Link to="/roadmap" className="block h-full">
                    <Card className="h-full flex flex-col items-center text-center hover:border-amber-400 dark:hover:border-amber-500 transition-all duration-300 shadow-lg hover:shadow-xl transition-shadow gap-4" hoverEffect>
                       <div className="w-16 h-16 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 dark:text-amber-300 rounded-2xl flex items-center justify-center mb-6 shadow-sm gap-4">
                          <Zap className="w-8 h-8" />
                       </div>
                       <Typography variant="h3" as="h3">Feature Requests</Typography>
                       <Typography variant="caption" className="text-slate-500 dark:text-slate-400 dark:text-slate-300">Missing a feature? Submit a request or vote on our public roadmap.</Typography>
                       <span className="text-amber-600 dark:text-amber-400 dark:text-amber-300 font-bold text-sm">View Roadmap &rarr;</span>
                    </Card>
                 </Link>
                 
                 <Link to="/contact" className="block h-full">
                    <Card className="h-full flex flex-col items-center text-center hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 shadow-lg hover:shadow-xl transition-shadow gap-4" hoverEffect>
                       <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 dark:text-blue-300 rounded-2xl flex items-center justify-center mb-6 shadow-sm gap-4">
                          <Shield className="w-8 h-8" />
                       </div>
                       <Typography variant="h3" as="h3">Premium Support</Typography>
                       <Typography variant="caption" className="text-slate-500 dark:text-slate-400 dark:text-slate-300">Priority support channel for Enterprise and Partner plans. 24/7 SLAs available.</Typography>
                       <span className="text-blue-600 dark:text-blue-400 dark:text-blue-300 font-bold text-sm">Open Ticket &rarr;</span>
                    </Card>
                 </Link>
              </CardSlider>
           </div>
